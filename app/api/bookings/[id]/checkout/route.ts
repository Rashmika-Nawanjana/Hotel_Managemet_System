  ...
  try {
    ...
    if (booking.status !== 'CheckedIn') {
      return NextResponse.json({ 
        error: 'Must be checked in to check out' 
      }, { status: 400 });
    }
    ...
    if (paidAmount < totalAmount) {
      return NextResponse.json({ 
        error: 'All payments must be completed before check-out',
        outstanding: totalAmount - paidAmount
      }, { status: 400 });
    }

    // Update booking status and check-out timestamp
    await pool.query(
      `UPDATE bookings 
       SET status = 'CheckedOut', checked_out_at = CURRENT_TIMESTAMP
       WHERE id = $1`,
      [bookingId]
    );
    ...
    return NextResponse.json(
      { error: 'Failed to check out', details: (error as Error).message },
      { status: 500 }
    );
  }
}
