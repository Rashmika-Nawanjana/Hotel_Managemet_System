...
export async function GET(request: NextRequest) {
  try {
    ...
    const section = url.searchParams.get('section') || 'all' // 'all', 'pending', 'confirmed'
    ...
    if (section === 'pending') {
      ...
      statusFilter = 'AND b.status = \'CONFIRMED\' AND b."paymentStatus" = \'PAID\''
    } else {
      // 'all' - show all bookings
      statusFilter = 'AND b.status IN (\'PENDING\', \'CONFIRMED\', \'CHECKED_IN\')'
    }
    ...
    const totalCount = await queryOne(`
      ...
      WHERE b.status IN ('PENDING', 'CONFIRMED')
        AND DATE(b."checkOutDate") > $${branchParams.length + 1}
        ${branchFilter}
        ${statusFilter}
    `, [...branchParams, today])
    ...
    const bookings = await query(`
      SELECT 
        ...
        CASE 
          WHEN DATE(b."checkInDate") < $${branchParams.length + 2} THEN true
          ELSE false
        END as is_overdue_checkin,
        CASE 
          WHEN DATE(b."checkInDate") = $${branchParams.length + 2} THEN true
          ELSE false
        END as is_due_today
      ...
      WHERE b.status IN ('PENDING', 'CONFIRMED')
        ...
        ${statusFilter}
      ORDER BY 
        CASE WHEN DATE(b."checkInDate") < $${branchParams.length + 2} THEN 0 ELSE 1 END,
        b."checkInDate" ASC
      LIMIT $${branchParams.length + 3} OFFSET $${branchParams.length + 4}
    `, [...branchParams, today, today, limit, offset])
    ...
    const transformedBookings = bookings.map(booking => ({
      ...
      isOverdueCheckin: booking.is_overdue_checkin,
    ...
    return NextResponse.json({ error: 'Failed to fetch check-in bookings' }, { status: 500 })
  }
}
